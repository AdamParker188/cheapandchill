...
const formatDateRangeHu = (startISO, endISO) => {
  ...
  const start = parseISO(String(startISO));
  const end = parseISO(String(endISO));
...
const formatFt = (value) => {
  ...
  if (typeof value === "string") {
    ...
    if (digits) {
      return new Intl.NumberFormat("hu-HU").format(Number(digits)) + " Ft";
    }
  }
  return "";
};
  ...
  return (
    <div className="flex flex-col h-full group">
      ...
      <Link
        to={`/csomag/${trip.id}`}
        state={{ trip }}
        className="block overflow-hidden rounded-lg aspect-[4/3] mb-4"
      >
        <img
          src={trip.image}
          alt={trip.title}
